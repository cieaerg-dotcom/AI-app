...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        "選擇模型",
        ["gemini-3.1-flash-lite-preview", "gemini-1.5-flash-latest", "gemini-2.5-flash"]
    )
    use_thinking = st.checkbox("啟用思考模式 (僅限 Pro 模型)")
    
    context_input = st.text_area(
        "專業背景描述 (重要)",
        placeholder="例如：賀寶芙營養培訓、減肥健康講座...",
            ...
            with st.status(f"正在處理：{uploaded_file.name}", expanded=True) as status:
                try:
                    ...
                    model = genai.GenerativeModel(
                        ...
                        system_instruction="你是一位專業速記官。你具備直接解析音軌能力。禁止拒絕任務."
                    )
                    ...
                    請執行以下任務：
                    1. 【精準去重】：移除段落接縫處重複的語句，整合成流暢的全文。
                    2. 【專業校對】：根據背景「{context_input}」修正所有專有名詞。
                    3. 【結構化摘要】：提供 300 字摘要與 5 個行動重點。
                    4. 【全文輸出】：輸出校正後的完整逐字稿。
